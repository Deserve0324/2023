...
function toggleMusic1(toggle) {
  if (typeof toggle === 'boolean') {
    store.setState({ music: toggle })
  } else {
    store.setState({ music: !store.state.music })
	musicPlay(!pp)
  }
}

var op;
function toggleDis(toggle) {
  if (typeof toggle === 'boolean') {
    store.setState({ dis: toggle })
  } else {
    store.setState({ dis: !store.state.dis })
  }
  zi(store.state.dis)
}
...
const musicEnabledSelector = (state = store.state) => state.music
...
const canPlayMusicSelector = (state = store.state) =>
  isRunning(state) && musicEnabledSelector(state)
...
const appNodes = {
  ...
  longExposureSVG: '#long-exposure use',
  disSVG: '#display-btns use',
...
function renderApp(state) {
  ...
  const soundBtnIcon = `#icon-sound-${musicEnabledSelector() ? 'on' : 'off'}`
  appNodes.pauseBtnSVG.setAttribute('href', pauseBtnIcon)
  appNodes.pauseBtnSVG.setAttribute('xlink:href', pauseBtnIcon)
  appNodes.soundBtnSVG.setAttribute('href', soundBtnIcon)
  appNodes.soundBtnSVG.setAttribute('xlink:href', soundBtnIcon)
  appNodes.longExposureSVG.setAttribute('href', `#icon-shutter-${state.longExposure ? 'fast' : 'slow'}`)
  appNodes.longExposureSVG.setAttribute('xlink:href', `#icon-shutter-${state.longExposure ? 'fast' : 'slow'}`)
  appNodes.disSVG.setAttribute('href', `#icon-display-${state.dis ? 'on' : 'off'}`)
  appNodes.disSVG.setAttribute('xlink:href', `#icon-display-${state.dis ? 'on' : 'off'}`)
...
function handlePointerStart(event) {
  ...
  if (event.y < btnSize) {
	...
	if (event.x > btnSize && event.x < 2*btnSize) {
		toggleLongExposure()
		return;
	}
    if (event.x > 4*btnSize && event.x < 5*btnSize) {
      toggleMenu()
      return
    }
	if (event.x > 3*btnSize && event.x < 4*btnSize) {
	  toggleDis()
	  return
	}
...
function render(speed) {
  ...
  trailsCtx.fillStyle = `rgba(0, 0, 0, ${
    store.state.longExposure ? 0.0025 : 0.175 * speed
  })`
...
shut()
function shut(){
	appNodes.longExposureSVG.addEventListener('click',toggleLongExposure)
	//appNodes.longExposureSVG.addEventListener('touchstart', toggleLongExposure);
}
//sound()
function sound(){
	toggleMusic1()
	//appNodes.soundBtnSVG.addEventListener('touchstart', toggleMusic1)
}
...
$('#yhBtn').click(e => {
    ...
    function start () {
		op=false
      let str = "炮声点燃声声起💥<br><br>二零二三将来临💫<br><br>此刻💥<br><br>满屏烟火向星辰⭐<br><br>万事所愿皆成真🍗<br><br>2023💫<br><br>平安喜樂，萬事勝意，六六大順🔆<br><br>祝你，祝我，祝ta，祝我們💖<br><br>除夕兼新年快乐💥<br><br>best wish 兔 us☘️<br><br><br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;SUPERL😎"
      let str_ = ''
      let i = 0
	...
	//var medip = document.getElementById("audioDom");
	var medip = document.querySelector('#audioDom');
	medip.addEventListener("volumechange", setHalfVolume());
